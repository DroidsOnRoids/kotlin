...
compileTestKotlin {
    ...
    private fun assertAllModulesConfigured() {
        runReadAction {
            for ((module, sourceRootModules) in ModuleSourceRootMap(myProject).groupByBaseModules(myProject.allModules())) {
                val configurator = allConfigurators().find {
                    it.getStatus(module, sourceRootModules) == ConfigureKotlinStatus.CAN_BE_CONFIGURED }
                Assert.assertNull("Configurator $configurator tells that $module can be configured", configurator)
            }
        }
    }
}
